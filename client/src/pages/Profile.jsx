...
export default function Profile() {
  const { user } = useAuth();
  const { badges, getSectionProgress, getOverallProgress } = useProgress();
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="card mb-8">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          ...
          <div className="text-center">
            ...
            <div className="text-sm text-text-muted mt-0.5">{overall.completed}/{overall.total} subsections</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div key={section.id} className="card">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-1">
                    {section.subsections.map(sub => {
                      const done = prog.completed > 0 && section.subsections.slice(0, prog.completed).some(s => s.id === sub.id);
                      return (
                        <div
                          key={sub.id}
                        ...
                        >
                          <CheckCircle
                            ...
                            style={{ color: done ? '#10B981' : undefined, opacity: done ? 1 : 0.3 }}
                          />
                          <span className={done ? 'text-text-primary' : 'opacity-60'}>{sub.title}</span>
                        </div>
                      );
                    })}
                  </div>
        ...
        <div>
          ...
          <div className="card">
            <BadgeGrid badges={badges} emptyMessage="Complete subsections to earn badges!" />
          </div>
        </div>
      </div>
    </div>
